...
impl ParseTableAction {
    pub fn is_shift(&self) -> bool {
        if let ParseTableAction::Shift(_) = self {
            true
        } else {
            false
        }
    ...
    pub fn is_reduce(&self) -> bool {
        if let ParseTableAction::Reduce(_) = self {
            true
        } else {
            false
        }
        ...
        for (i, state) in collection.states.iter() {
            if state.is_accept(grammar) {
                ...
                for final_item in state.get_items().iter().filter(|i| i.is_final(grammar)) {
                    for terminal in reduce_terminal_set_fn(&final_item, state, *i).iter() {
                        table.insert_entry(
                            *i,
                            &*terminal,
            ...
            // Shift / Goto
            for edge in collection.edges.get(i).unwrap_or(&Vec::new()).iter() {
                table.insert_entry(
                    *i,
                    &*edge.symbol,
...
impl ParseTableLR {
    pub fn apply_conflict_resolutions(&mut self, overrides: Vec<ParseTableOverride>) {
        // Resolve S/R or R/R conflicts
        for o in overrides {
            self.insert_entry(o.state, o.symbol, o.action);
        }
    }
    ...
    pub fn insert_entry(&mut self, state_index: usize, symbol: &str, action: ParseTableAction) {
        match self.table.get_mut(symbol) {
            Some(v) => {
                if let Some(old_action) = v.insert(state_index, action) {
                    println!("{} / {}", old_action, action);
                }
            }
            None => {
                self.table
    ...
    pub fn to_prettytable(&self) -> Table {
        ...
        let mut titles: Vec<Cell> = symbols
            .iter()
        ...
        for state_index in 0..self.num_states {
            ...
            for (symbol_index, symbol) in symbols.iter().enumerate() {
                ...
                actions[symbol_index + 1] = Cell::new(action.to_string().as_str()).style_spec("c");
                match action {
                    ParseTableAction::Accept => {
                        actions[symbol_index + 1].style(Attr::ForegroundColor(GREEN))
                    }
                    ParseTableAction::Shift(_) => {
                        actions[symbol_index + 1].style(Attr::ForegroundColor(BRIGHT_BLUE))
                    }
                    ParseTableAction::Reduce(_) => {
                        actions[symbol_index + 1].style(Attr::ForegroundColor(BRIGHT_MAGENTA))
                    }
                    ParseTableAction::Goto(_) => {
                        actions[symbol_index + 1].style(Attr::ForegroundColor(CYAN))
                    }
                    _ => {}
                }
            }
